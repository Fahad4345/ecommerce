...
import { InsertCart } from './../../../Api/Cart/insertCart';
import { useRouter } from "next/navigation";
import { useSelector } from "react-redux";

export default function ProductDetail({ cartItem }) {
    ...
    if (!item) {
        return (
            <div>
                ...
                <p className="mt-20 text-center text-gray-500">Loading item...</p>
            </div>
        );
    }
    ...
    return (
        <div className=' flex justify-center items-center flex-col '>
            ...
            <div className=' mx-[135px] mt-[80px]  max-w-[1170px] w-full'>
                ...
                <div className='flex gap-[70px]'>
                    <div className=' flex gap-[30px]'>
                        <div className=' flex flex-col gap-[16px] '>{smallImages?.map((img, idx) => (<div key={idx} className=' w-[170px] h-[138px]  flex justify-center items-center rounded-[4px]  bg-[#F5F5F5]'><Image src={img} width={121} height={114} alt="" /></div>))}</div>
                    ...
                    <div className=' flex flex-col'>
                        ...
                        <div className=' flex flex-col gap-[24px] mt-[24px]'>
                            <div className=' w-[373px]'><h1 className=' font-[Poppins] font-[400] text-[14px] leading-[21px] tracking-[0px]'>{item.description}</h1></div>
         ...
         ${selectedColor === color ? 'scale-110 border-black' : 'border-transparent'}`}
                                            style={{ backgroundColor: selectedColor === color ? 'transparent' : color }}
                                        >
                                            {selectedColor === color && (
                                                <div
                            ...
                            <div className=' flex justify-center items-center h-fit'>
                                ...
                                <button onClick={() => { InsertCart(item._id, size, selectedColor, quantity); router.push("/cart"); }} className='w-fit ml-[16px]  flex justify-center items-center text-[16px] bg-[#DB4444] text-white cursor-pointer rounded-[4px]'>
                                    <span className='px-[48px] py-[10px] font-[Poppins] font-[500] text-[16px] leading-[24px] tracking-[0%]'>Buy Now</span>
                                </button>
                                <div onClick={() => insertItem(item._id)} className=' flex justify-center items-center ml-[19px] border-[1px] border-[#00000080] w-[40px] h-[40px]'><Image src="/assets/icons/Wishlist.svg" width={32} height={32} alt="" /></div>
                            </div>
            ...
            {/* <SaleSection
                ...
                showViewAll={false}
                className=' mx-[135px] max-w-[1170px]'
